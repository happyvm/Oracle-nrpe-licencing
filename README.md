...
|---|---|---|
| `run_tests.sh` | 109 | les cinq modes, les seuils, l'absence de faux positifs, et les particularités de chaque version — 9i à 21c |
| `run_collector_tests.sh` | 32 | découverte via `oratab`, nommage des caches, écriture atomique, filtres, `--dry-run`, instance arrêtée |
| `run_parity_tests.sh` | 43 | verdicts identiques entre les trois moteurs |
...
`gawk`, `mawk` et `nawk` — soit **466 exécutions de test**. S'y ajoute
